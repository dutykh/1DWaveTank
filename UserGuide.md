...
- **`+cfg`**: Configuration files and functions, including bathymetry definitions (`+cfg/+bathy`).
...
- **New Bathymetry:** Add a function to `+cfg/+bathy/`, e.g., `h = my_bathy(x, cfg)` and use `bathyHandle = @cfg.bathy.my_bathy;`. Ensure well-balanced scheme support for non-flat beds.
